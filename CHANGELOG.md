...
- Replaced the initial market table with a link to the live Bungee assets page for supported markets, fixed leverage, and position limits.
...
- Set a $20,000 USD gross payout allowance per rolling 24-hour period for each funded account, regardless of account size or track, before the 80/20 profit split.
- Allowed additional payout requests while rolling allowance remains, with each processed payout releasing its allowance after 24 hours; updated timing, examples, and eligibility accordingly.
...
- Removed the version, status, and last-updated header block from the rulebook.
